...
sys.path.append('D:/study_peoject/zhxz_model/')
...
def update_aj_jbxx(num=10):
    ...
    for i in range(num):
        ...
        slsj = faker.date_between_dates(now + datetime.timedelta(days=-1),now)
...
def update_rx_gj(num=100):
    ...
    for i in range(num):
        ...
        dict_["passtime"] = faker.date_time_between(now + datetime.timedelta(days=-1),now)
        list_.append(dict_)
...
def update_gj_lgzs(num=100):
    ...
    data_ry = get_data_from_db(sql="select userid from ry_jbxx",conn=conn_mysql)
    list_ry = list(set(data_ry["userid"]))
    list_hour = [random.randint(8,18)] + [random.randint(18,24)]*10 + [random.randint(0,6)]*5
    list_hour = ['0' + str(i) for i in list_hour if i<10] + [str(i) for i in list_hour if i>=10]
    def gen_guestogeid():
        str_togeid = random.choice(list_device) + re.sub('-| |:','',str(faker.date_time_between(now+datetime.timedelta(days=-1),now)))
    ...
    data_gj_lgzs.loc["rksj"] = str(datetime.datetime.now())[0:19]
